...
- **Operator-led interactive recording (`record`)**: a person controls a visible
  ...
  workflow is available from the **command line only for now**.
...
|---|---|---|---|
| Interactive recording | The operator | `headed`, `native` | Command line only |
...
This feature currently runs from the command line. It is **not yet exposed through
the SWM dashboard**.
